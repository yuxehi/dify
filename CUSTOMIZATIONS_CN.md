...
管理员创建知识库时使用受权限保护的 `scope=all` 查询：

```text
GET /console/api/fileList?scope=all
```

只有 Workspace Owner/Admin 可以使用该范围。后端向既有教学平台函数传递空邮箱过滤，从而返回所有任务的全部已有文件；Editor 学生即使手工构造该请求也会收到 403。
...
学生选择的是平台已经存在的 `UploadFile.id`，不会重复上传。管理员在官方本地上传区域下方同时拥有教学平台文件入口，并可选择所有任务的已有文件；两种来源都位于左侧，右侧继续使用官方文件预览。删除知识库/文档时保留原始平台文件；1.14.2 新增的分段图片和附件仍按官方逻辑清理，避免 1.0.1 那样无限保留全部衍生文件。
