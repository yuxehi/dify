...
- Python 要求：官方 1.14.2 要求 Python `~=3.12.0`。当前本机 `dify-env` 是 3.12.13，但尚未安装完整 API 测试依赖。
- GitHub 提交、钩子、C 盘缓存和镜像发布交接说明：[`GITHUB_PUBLISH_GUIDE_CN.md`](./GITHUB_PUBLISH_GUIDE_CN.md)。
...
教师端与学生端均保持 1.0.1 的行为：教学平台文件入口只查询当前登录账号邮箱对应的文件。教师端另外保留 Dify 官方本地上传入口，但不会聚合其他 Workspace 成员的教学平台文件。1.14.2 后端增加了邮箱归属校验，因此任何账号手工构造其他成员邮箱的请求都会收到 403。
...
学生选择的是平台已经存在的 `UploadFile.id`，不会重复上传。管理员在官方本地上传区域下方同时拥有教学平台文件入口，该入口与 1.0.1 一样只展示管理员当前邮箱对应的文件；两种来源都位于左侧，右侧继续使用官方文件预览。删除知识库/文档时保留原始平台文件；1.14.2 新增的分段图片和附件仍按官方逻辑清理，避免 1.0.1 那样无限保留全部衍生文件。
...
- Workflow 添加工具隐藏“精选推荐”和“在 Marketplace 中查找更多”；选择触发器时同样不向学生请求或展示“精选推荐”，并隐藏底部“在 Marketplace 中查看更多”。
...
- 普通知识库创建页保留官方左右分屏：学生只从教学平台选择课程文件，可查看、移除和重新选择已选文件，点击文件后在右侧预览，再进入分段与索引设置。
- “文本分段与清洗”的分隔符、最大长度和重叠长度输入项在窄窗口下自动换行，并为数值、`characters` 单位及步进按钮保留最小宽度；该修复对所有用户生效。
- 顶部主导航在任何窗口宽度下都保留文字：所有用户保留“工作室、知识库”，Owner/Admin 同时保留“探索、工具”；641–1024px 宽度下导航自动换到独立第二行，避免图标化或相互挤压。
- 学生应用卡片的“创建空白应用”和知识库卡片的“创建知识库”沿用 Dify 1.0.1 的卡片效果：前者为无额外边框的上下居中图标与文字，后者为左上虚线图标框、标题和底部说明文字；管理员的多入口卡片继续使用 1.14.2 紧凑列表。
- “创建空白应用”弹窗中的“新手适用”应用类型对所有角色默认展开，同时保留手动收起能力。
...
1. 按 `api/uv.lock` 为 Python 3.12 的 `dify-env` 安装完整依赖。
...
10. 验证学生文件列表只能查询本人；验证 Owner/Admin 能从原始上传与教学平台全部任务文件两种来源创建知识库。
...
- `dify-env` 检查：确认 Python 3.12.13；当前缺少 `pytest`、`flask-restx` 等完整 API 测试依赖。
...
- 受影响的发布权限、知识库入口、Step One、文件选择、窄屏分段设置与顶部导航组件测试：190 项通过。
- 教学文件本人/管理员全部任务范围权限测试：2 项通过；相关 Python 文件 Ruff 与格式检查通过。
